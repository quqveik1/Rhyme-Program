//=================================================================================================================
//           [These sections are for folding control  in Code::Blocks]         [$Date: 2021-01-23 10:12:28 +0400 $]
...
#define _TX_VER      _TX_v_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 169, 2021-01-23 10:12:28 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_VERSION  _TX_V_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 169, 2021-01-23 10:12:28 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
#define _TX_AUTHOR   _TX_A_FROM_CVS ($VersionInfo: , TXLib.h, 00173a, 169, 2021-01-23 10:12:28 +0400, "Ded (Ilya Dedinsky, http://txlib.ru) <mail@txlib.ru>", $)
...
//!
//!          Если константа @ref TX_CONSOLE_MODE задана равной @c SW_FORCEMINIMIZE и при старте программы создалось
//!          новое консольное окно (например, при запуске неконсольного приложения Windows), то это окно будет
//!          автоматически свернуто, чтобы избежать неприятного мигания при открытии графического окна с помощью
//!          @ref txCreateWindow(). После открытия графического окна консоль будет скрыта, как если бы константа
//!          @ref TX_CONSOLE_MODE была равна @c SW_HIDE.
//!
...
//!          txCreateWindow (800, 600);
//!
//!          #define TX_CONSOLE_MODE  SW_FORCEMINIMIZE // Скрыть консольное окно с самого начала программы:
//!          #include "TXLib.h"                        // Сначала свернуть его, а при открытии графического окна - сжечь*.
//!          ...                                       // Сработает только для неконсольных приложений, см. выше.
//!          txCreateWindow (800, 600);
//!
//!          #define TX_CONSOLE_MODE  SW_SHOW          // Всегда показывать консольное окно, например, для отладки
//!          #include "TXLib.h"
//!          ...
//!          txCreateWindow (800, 600);                                                              // * скрыть, конечно)
//! @endcode
//!
//! @hideinitializer
//}----------------------------------------------------------------------------------------------------------------
...
$   if (console && (proc == GetCurrentProcessId() || _txIsParentWaitable()))
        {$ ShowWindow (console, ((TX_CONSOLE_MODE) == SW_FORCEMINIMIZE)? SW_HIDE : (TX_CONSOLE_MODE)+0); }
...
$   if (!console)
        {
$       bool minimizeConsole = ((TX_CONSOLE_MODE) == SW_FORCEMINIMIZE && !_txIsConsoleSubsystem());

$       const char consoleHive[] = "Console";
$       const char consolePos[]  = "WindowPosition";

$       int   noCoord = -32000;
$       DWORD noPos   = MAKELONG (noCoord, noCoord);

$       DWORD oldPos  = noPos;
$       DWORD size    = sizeof (oldPos);

$       if (minimizeConsole) do
            {
$           HKEY key = NULL;
$           LRESULT err = RegOpenKeyEx (HKEY_CURRENT_USER, consoleHive, 0, KEY_ALL_ACCESS, &key);
            if (err) {$ break; }

$           err = RegQueryValueEx (key, consolePos, NULL, NULL, (BYTE*) &oldPos, &size);
            if (err) {$ oldPos = noPos; }

$           RegSetValueEx (key, consolePos, 0, REG_DWORD, (BYTE*) &noPos, sizeof (noPos));
...
$       if (minimizeConsole) do
            {
$           if (console && newConsole)
                {
$               ShowWindow (console, SW_MINIMIZE);

$               WINDOWPLACEMENT place = {};
$               GetWindowPlacement (console, &place);

$               RECT rect = place.rcNormalPosition;

$               if (rect.left == noCoord && rect.top == noCoord)
                    {
$                   POINT pos = { (oldPos != noPos)? LOWORD (oldPos) : 100,
                                  (oldPos != noPos)? HIWORD (oldPos) : 100 };

$                   RECT  rc  = { pos.x, pos.y, pos.x + (rect.right  - rect.left),
                                                pos.y + (rect.bottom - rect.top) };
$                   place.rcNormalPosition = rc;

$                   SetWindowPlacement (console, &place);
                    }
                }
...
$           LRESULT err = RegOpenKeyEx (HKEY_CURRENT_USER, consoleHive, 0, KEY_ALL_ACCESS, &key);
            ...
            if (oldPos != noPos) {$ RegSetValueEx  (key, consolePos, 0, REG_DWORD, (BYTE*) &oldPos, sizeof (oldPos)); }
            else                 {$ RegDeleteValue (key, consolePos); }
